...
__global__ void merge_tiled_gpu(const int *__restrict__ A,
  ...
  int A_S_start             = 0;
  int B_S_start             = 0;
  // Number of elements that must be loaded
  ...
  while (counter < total_iteration) {
    const int C_missing      = C_length - C_completed;
    const int C_missing_tile = gpu_min(TILE_SIZE, C_missing);
    const int A_missing      = A_length - A_consumed;
    const int A_missing_tile = gpu_min(TILE_SIZE, A_missing);
    const int B_missing      = B_length - B_consumed;
    const int B_missing_tile = gpu_min(TILE_SIZE, B_missing);
    // Loading (refilling) A_S_consumed elements into A_S
    for (int i = threadIdx.x; i < A_S_consumed; i += blockDim.x) {
      if (i < A_missing && i < A_S_consumed) {
        A_s[(A_S_start + (TILE_SIZE - A_S_consumed) + i) % TILE_SIZE] = A[A_curr + i + A_read];
      }
    }
    A_read += gpu_min(A_missing, A_S_consumed);

    // Loading B_S_consumed elements into B_S
    for (int i = threadIdx.x; i < B_S_consumed; i += blockDim.x) {
      if (i < B_missing && i < B_S_consumed) {
        B_s[(B_S_start + (TILE_SIZE - B_S_consumed) + i) % TILE_SIZE] = B[B_curr + i + B_read];
      }
    }
    B_read += gpu_min(B_missing, B_S_consumed);
    __syncthreads();

    const int c_curr = gpu_min(threadIdx.x * (TILE_SIZE / blockDim.x), C_missing);
    const int c_next = gpu_min((threadIdx.x + 1) * (TILE_SIZE / blockDim.x), C_missing);

    // Find co-rank for c_curr and c_next
    int a_curr =
        co_rank_circular(c_curr, A_s, A_missing_tile, B_s, B_missing_tile, A_S_start, B_S_start, TILE_SIZE);
    int b_curr = c_curr - a_curr;

    int a_next =
        co_rank_circular(c_next, A_s, A_missing_tile, B_s, B_missing_tile, A_S_start, B_S_start, TILE_SIZE);
    int b_next = c_next - a_next;

    // Do merge in parallel
    merge_sequential_circular(A_s,
                              a_next - a_curr,
                              B_s,
                              b_next - b_curr,
    ...
    counter++;
    A_S_consumed = co_rank_circular(C_missing_tile,
                                    A_s,
                                    A_missing_tile,
                                    B_s,
                                    B_missing_tile,
    ...
    B_S_consumed = C_missing_tile - A_S_consumed;

    A_consumed += A_S_consumed;

    C_completed += C_missing_tile;
...
int main() {
  ...
  for (int i = 0; i < SIZE_A; ++i) { A_host[i] = rand() % 100000; }
  for (int i = 0; i < SIZE_B; ++i) { B_host[i] = rand() % 100000; }
