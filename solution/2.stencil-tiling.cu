...
__global__ void stencil_kernel_tiling_gpu(const float *__restrict__ in, float *__restrict__ out, const int N)
{
  ...
  {
    if (threadIdx.z >= 1 && threadIdx.z < IN_TILE_DIM &&
        threadIdx.y >= 1 && threadIdx.y < IN_TILE_DIM &&
        threadIdx.x >= 1 && threadIdx.x < IN_TILE_DIM)
    {
      get(out, i, j, k, N) = c0 * in_s[threadIdx.z][threadIdx.y][threadIdx.x] + c1 * in_s[threadIdx.z][threadIdx.y][threadIdx.x-1] +
