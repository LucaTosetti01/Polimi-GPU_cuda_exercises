...
__device__ __host__ void merge_cpu(const int *A, const int dim_A, const int *B, const int dim_B, int *C)
{
    int i = 0;
    int j = 0;
    int k = 0;
...
__global__ void merge_gpu(const int *A, const int dim_A, const int *B, const int dim_B, int *C)
{
    ...
    merge_cpu(&A[i_curr], i_next - i_curr, &B[j_curr], j_next - j_curr, &C[k_curr]);
}
...
int main()
{
    ...
    merge_cpu(A_host.data(), SIZE_A, B_host.data(), SIZE_B, cpu_result.data());
