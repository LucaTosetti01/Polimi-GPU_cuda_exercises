...
void __global__ cenergy_gpu(float *energygrid) {
  const int i = blockIdx.x * blockDim.x * COARSEN_FACTOR + threadIdx.x * COARSEN_FACTOR;
  ...
  for (int t = 0; t < GRID_DIM; t++) {
    ...
    for (int n = 0; n < NUM_ATOMS * ATOM_STRIDE; n += ATOM_STRIDE) {
      const float dx0      = x - d_atoms[n];
      const float dx1      = dx0 + GRID_SPACING;
      const float dx2      = dx0 + 2 * GRID_SPACING;
      const float dx3      = dx0 + 3 * GRID_SPACING;
      const float dy       = y - d_atoms[n + 1];
      const float dz       = z - d_atoms[n + 2];
      const float dysqdzsq = dy * dy + dz * dz;
    ...
    energygrid[GRID_DIM * GRID_DIM * t + GRID_DIM * j + i] += energy0;
    energygrid[GRID_DIM * GRID_DIM * t + GRID_DIM * j + i + 1] += energy1;
    energygrid[GRID_DIM * GRID_DIM * t + GRID_DIM * j + i + 2] += energy2;
    energygrid[GRID_DIM * GRID_DIM * t + GRID_DIM * j + i + 3] += energy3;
    __syncthreads();
  }
}
