...
void __global__ cenergy_gpu(float *energygrid)
{
  ...
  const float atom_x = d_atoms[n];
  const float atom_y = d_atoms[n + 1];
  const float atom_z = d_atoms[n + 2];
  for (int t = 0; t < GRID_DIM; t++)
  {
    const float z = GRID_SPACING * (float)t;
    const float dz = z - atom_z; // all grid points in a slice have the same
    const float dz2 = dz * dz;   // z value
    // starting position of the slice in the energy grid
    const int grid_slice_offset = (GRID_DIM * GRID_DIM * z) / GRID_SPACING;
    for (int j = 0; j < GRID_DIM; j++)
    {
      const float y = GRID_SPACING * (float)j;
      const float dy = y - atom_y; // all grid points in a row have the same
      const float dy2 = dy * dy;   // y value
