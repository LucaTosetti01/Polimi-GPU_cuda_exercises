...
void __global__ cenergy_gpu(float *energygrid) {
  ...
  for (int t = 0; t < GRID_DIM; t++) {
    ...
    for (int n = 0; n < NUM_ATOMS * ATOM_STRIDE; n += ATOM_STRIDE) {
      ...
      const float dx1      = dx0 + blockDim.x * GRID_SPACING;
      const float dx2      = dx0 + 2 * blockDim.x * GRID_SPACING;
      const float dx3      = dx0 + 3 * blockDim.x * GRID_SPACING;
    ...
    energygrid[GRID_DIM * GRID_DIM * t + GRID_DIM * j + i + blockDim.x] += energy1;
    energygrid[GRID_DIM * GRID_DIM * t + GRID_DIM * j + i + 2 * blockDim.x] += energy2;
    energygrid[GRID_DIM * GRID_DIM * t + GRID_DIM * j + i + 3 * blockDim.x] += energy3;
    __syncthreads();
  }
}
