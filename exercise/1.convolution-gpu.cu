...
__global__ void convolution_gpu(input_type *input, const input_type *filter, input_type *output, const int width, const int height, const int filter_size, const int filter_radius)
{
...
int main(int argc, char **argv)
{
    ...
    convolution_gpu<<<blocksPerGrid, threadsPerBlock>>>(d_input, d_filter, d_output, width, height, FILTER_SIZE, FILTER_RADIUS);
    ...
    for (int i = 0; i < height; i++)
    {
        ...
        {
            if (output_gpu[i * width + j] != output_cpu[i * width + j])
            {
