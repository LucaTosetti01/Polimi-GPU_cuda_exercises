...
void BFS_sequential(const int source, const int *rowPointers, const int *destinations, int *distances) {
  ...
  for (int i = 0; i < 2; i++) frontiers_host[i] = (int *) calloc(MAX_FRONTIER_SIZE, sizeof(int));
  ...
  if (t < *previousFrontierSize) {
    ...
    for (int i = rowPointers[vertex]; i < rowPointers[vertex + 1]; ++i) {
      ...
      if (!alreadyVisited) {
        ...
        if (sharedQueueIndex < MAX_FRONTIER_SIZE) { // there is space in the local queue
  ...
  while (hPreviousFrontierSize > 0) {
    ...
    if (numBlocks < NUM_BLOCKS_THRESHOLD) {
      if (currentDataOnDevice) {
        CHECK(cudaMemcpy(hPreviousFrontier,
                         dPreviousFrontier,
                         hPreviousFrontierSize * sizeof(int),
                         cudaMemcpyDeviceToHost));
        CHECK(cudaMemcpy(&hCurrentFrontierSize, dCurrentFrontierSize, sizeof(int), cudaMemcpyDeviceToHost));
        CHECK(cudaMemcpy(hCurrentFrontier,
                         dCurrentFrontier,
                         hCurrentFrontierSize * sizeof(int),
                         cudaMemcpyDeviceToDevice));
        CHECK(cudaMemcpy(distances, dDistances, num_vals * sizeof(int), cudaMemcpyDeviceToHost));
        CHECK(cudaMemcpy(hVisited, dVisited, num_vals * sizeof(int), cudaMemcpyDeviceToHost));
      ...
      BFS_iterate_sequential(&hPreviousFrontier,
                             ...
                             destinations,
                             distances);
      for (int i = 0; i < hPreviousFrontierSize; i++) hVisited[hPreviousFrontier[i]] = 1;
    } else {
      if (!currentDataOnDevice) {
        CHECK(cudaMemcpy(dPreviousFrontierSize, &hPreviousFrontierSize, sizeof(int), cudaMemcpyHostToDevice));
        CHECK(cudaMemcpy(dPreviousFrontier,
                         hPreviousFrontier,
                         hPreviousFrontierSize * sizeof(int),
                         cudaMemcpyHostToDevice));
        CHECK(cudaMemcpy(dCurrentFrontierSize, &hCurrentFrontierSize, sizeof(int), cudaMemcpyHostToDevice));
        CHECK(cudaMemcpy(dCurrentFrontier,
                         hCurrentFrontier,
                         hCurrentFrontierSize * sizeof(int),
                         cudaMemcpyHostToDevice));
        CHECK(cudaMemcpy(dDistances, distances, num_vals * sizeof(int), cudaMemcpyHostToDevice));
        CHECK(cudaMemcpy(dVisited, hVisited, num_vals * sizeof(int), cudaMemcpyHostToDevice));
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < num_vals; i++) { dist_gpu[i] = -1; }
  // Compute on the gpu
