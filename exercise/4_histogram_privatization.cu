...
/*__global__ void histogramGPU(char *data, unsigned int *histogram, int length)
{
  ...
  for (int i = tid; i < length; i += stride)
  {
    int alphabet_position = data[i] - 'a';
    if (alphabet_position >= 0 && alphabet_position < 26) // check if we have an alphabet char
      atomicAdd(&(private_histogram[alphabet_position / CHAR_PER_BIN]), 1);
  }
...
__global__ void histogramGPU(char *data, unsigned int *histogram, int length)
{
  ...
  unsigned int register_histogram[NUM_BINS];
  // initializing shared memory
  for (unsigned int j = 0; j < NUM_BINS; j ++)
  {
    register_histogram[j] = 0u;
  }
  ...
  for (int i = tid; i < length; i += stride)
  {
    int alphabet_position = data[i] - 'a';
    if (alphabet_position >= 0 && alphabet_position < 26) // check if we have an alphabet char
      register_histogram[alphabet_position / CHAR_PER_BIN] += 1;
  }

  //Commit to global memory
  for (unsigned int j = 0; j < NUM_BINS; j ++)
  {
    const int value = register_histogram[j];
    if (value > 0)
    {
      atomicAdd(&histogram[j], value);
    }
...
int main(int argc, char *argv[])
{
  ...
  dim3 blocksPerGrid(GRID_DIM, 1, 1);
