                             ...
                             float *y)
{
  ...
  // for (int i = 0; i < num_elements; i += stride)   //In case we don't dynamically adapt the number of blocks in the grid
  if (tx < num_elements)
  {
    int row = row_indices[tx];
    int col = column_indices[tx];

    atomicAdd(&y[row], values[tx] * x[col]);
  }
}
